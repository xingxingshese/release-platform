...
> **状态盘点（2026-08-25 第二轮，后端补全完成）**：Phase 0~19 后端代码全部落地，`mvn verify` 147 tests 全绿（JDK 17）。本轮新增：
> - Phase 11/12：PreProdReleaseService（PRE/PROD 编排、并行开关、权限点、分布式锁）
> - Phase 13/14：DeploymentAdapter 抽象 + KubernetesAdapter(RestKubernetesSnapshotFetcher) + FrontendDeploymentAdapter + DeploymentVerificationService（release_deployment_node 逐节点落库，V10 迁移）
> - Phase 15：notification 模块（NotificationChannel/Rule V9 迁移、路由匹配、@Async 分发、WeCom/Feishu Webhook+Email Provider、失败不阻断、发送留痕）
> - Phase 17：admin 模块（ConfigVersion 版本递增保存、字段级 diff、AdminConfigController）
> - Phase 18：audit 模块（@AuditLog 切面 + SensitiveMasker 脱敏，operation_log 落库）
> - Phase 19：ReleaseMetrics 全集 + micrometer-prometheus（/actuator/prometheus）
> - specs：17 个规格目录 × SDD 七件套（requirements/design/api/data-model/test-plan/acceptance/README）
> 遗留缺口：
> ① ~~Docker 缺失~~ 已解决：`mvn verify` 121 tests 全绿（2026-08-25）；
> ② 前端骨架未验证（pnpm install/build）；
> ③ specs 目录缺 `000-foundation`、`005-jenkins`，且已有 spec 只有 README.md，不满足 SDD 七件套要求；
> ④ `ci/`（Jenkinsfile/Shared Library）、`tests/`（跨端 E2E）目录尚未创建；
> ⑤ Phase 16 Alert 的 escalation 指标埋点未接入 AlertEscalationJob；
> ⑥ Phase 20 E2E、Phase 21 Production Hardening（Helm/限流/压测/故障演练）未开始；
> - 前端（2026-08-25 第二轮）已实现：Element Plus+ECharts、MainLayout、发布列表/详情(Timeline+节点明细)、项目管理、需求管理、报警中心(ACK/恢复)、管理员配置中心(版本/diff)、useAsync 五态 + usePolling(5s)；`pnpm build` 通过。前端不走测试用例（用户指令）；
> ⑥ pom.xml 已补 UTF-8 sourceEncoding；构建须使用 JDK 17（`JAVA_HOME=D:/BellSoft/LibericaJDK-17`）。
